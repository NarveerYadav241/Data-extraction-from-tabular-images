...
def chunkIt(seq, num):
    ...
    return out
def multithreading_parser(tess_image,header_mean,queue,lock,que2):
    while not queue.empty():
        ...
        for line in work[1]:
            ...
            line_type = xml_data.attrib['type']
            line_number = int(xml_data.attrib['number'])-1
            blocks = xml_data.getchildren()
            lock.acquire()
            for block in blocks:
                block_number = int(block.attrib['number']) - 1
                ...
                if (parent_type == "lineHeader"):
                    ...
                    bordered_image = cv2.copyMakeBorder(segment_image, 5, 5, 5, 5, cv2.BORDER_CONSTANT,
                                                        value=(255, 255, 255))
                ...
                if (dataFill):
                    ...
                    fill_data[line_number][min_index] = info_list[0]
                    dataFill = False
                    # print(info_list[0])
            que2.put(fill_data)
            lock.release()
...
def multiprocessing_parser(tess_image,lines_group,header_mean,queue,header_position = 0):
    numpy_array = queue.get()
    for line in lines_group:
        line_number = int(line.attrib['number'])-1
        blocks = line.getchildren()
        for block in blocks:
            block_number = int(block.attrib['number'])-1
            block_startx = int(block.attrib['startX'])
            block_endx = int(block.attrib['endX'])
            ...
            if dataFill:
                ...
                for subvalue in subnp:
                    subarray.append(abs(subvalue))
                    min_index = subarray.index(min(subarray))
                numpy_array[line_number - 1][min_index] = info_list[0]
...
def data_to_fill_array(table_data_xml,header_position=0):
    ...
    for i in range(len(total_lines_including_header) - 1):
        ...
        if i == mean_line:
            ...
            for block in mean_blocks:
                header_mean.append(int(int(block.attrib['startX']) + int(
                    abs(int(block.attrib['endX']) - int(block.attrib['startX'])) / 2)))
            break
    data_to_fill = np.full((dataframe_rows, dataframe_cols), np.nan, dtype=object)
    return data_to_fill,header_mean
...
def without_tables_function(orig_image,header_select=0,thread_pool_size = 2):
    ...
    total_lines_including_header = table_xml.xpath("//image//table_data//table_line")
    data_to_fill,header_mean = data_to_fill_array(table_xml,header_position=header_select)
    all_lines_string = list()
    q1 = Queue()
    q2 = Queue()
    # thread_pool_size=len(total_lines_including_header)
    lock = Lock()
    for line in total_lines_including_header:
        all_lines_string.append(ET.tostring(line))
    chunk_lines = chunkIt(all_lines_string,thread_pool_size)
    for i in range(thread_pool_size):
        q1.put((i,chunk_lines[i]))
    q2.put(data_to_fill)
    for i in range(thread_pool_size):
        worker = Thread(target=multithreading_parser,args=(orig_image.copy(),header_mean,q1,lock,q2))
        worker.start()
